module 0x46566b4a16a1261ab400ab5b9067de84ba152b5eb4016b217187f2a2ca980c5a::emergency {
    ...
    public fun assert_no_emergency()
        acquires EmergencyAccountCap
    {
        assert!(!is_emergency(), 602);
    }
    public entry fun disable_forever(p0: &signer)
        acquires EmergencyAccountCap
    {
        assert!(!is_disabled(), 601);
        config::assert_emergency_admin(p0);
        let _v0 = account::create_signer_with_capability(&borrow_global<EmergencyAccountCap>(@0x46566b4a16a1261ab400ab5b9067de84ba152b5eb4016b217187f2a2ca980c5a).signer_cap);
        let _v1 = &_v0;
        let _v2 = IsDisabled{};
        move_to<IsDisabled>(_v1, _v2);
    }
    fun get_emergency_account_address(): address
        acquires EmergencyAccountCap
    {
        account::get_signer_capability_address(&borrow_global<EmergencyAccountCap>(@0x46566b4a16a1261ab400ab5b9067de84ba152b5eb4016b217187f2a2ca980c5a).signer_cap)
    }
    fun init_module(p0: &signer) {
        let (_v0,_v1) = account::create_resource_account(p0, vector[101u8, 109u8, 101u8, 114u8, 103u8, 101u8, 110u8, 99u8, 121u8, 95u8, 97u8, 99u8, 99u8, 111u8, 117u8, 110u8, 116u8]);
    ...
    public entry fun pause(p0: &signer)
        acquires EmergencyAccountCap
    {
        assert!(!is_disabled(), 601);
        assert_no_emergency();
    ...
    public entry fun resume(p0: &signer)
        acquires EmergencyAccountCap, IsEmergency
    {
        assert!(!is_disabled(), 601);
